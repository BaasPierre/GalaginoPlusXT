 ...
 *
 * Changelog:
 *   2026-09-02  m6809_step() now polls the interrupt lines once per
 *               instruction (see the big comment in m6809_step). Needed
 *               by the aristmk4/swtht2nz port; harmless for every other
 *               M6809 game. Old behaviour: no poll in the step loop, an
 *               interrupt was only vectored inside m6809_irq/firq/nmi().
 */
...
int m6809_step(m6809_state *s, char count) {
  ...
  for (int i=0; i < count; i++) {
    /* ------------------------------------------------------------------
       2026-09-02 (swtht2nz / Aristocrat MK IV port): poll the interrupt
       lines EVERY instruction, like real M6809 silicon.

       WHY: previously check_interrupts() was only ever called from inside
       m6809_irq() / m6809_firq() / m6809_nmi() - i.e. at the exact moment
       the machine pulses the line - and only vectored if the relevant
       mask (CC.I / CC.F) happened to be clear at that instant. A line
       raised while the CPU had it masked (ORCC #$10 around a critical
       section, or an ISR already running) was then never re-checked and
       the interrupt was lost.

       That was invisible for every existing M6809 game because they all
       pulse m6809_irq() exactly once per frame at a point where CC.I is
       clear. swtht2nz drives a ~1 kHz VIA-Timer-2 FIRQ from run_frame()
       (not on a CPU-instruction boundary) and its mainline spins on a
       RAM delay counter that only the FIRQ ISR increments - so a missed
       FIRQ = hard hang.

       IMPACT on the once-per-frame-pulse machines (circusc, gaplus,
       gyruss, mappy, rocnrope, todruaga, tutankhm): none functional -
       the pending bit still originates from their m6809_irq() call, it
       is just serviced on the next instruction instead of inside the
       call. Cost is one predicted-not-taken compare per instruction.

       OLD CODE (restore this block and delete the two lines below to
       revert):
           // for (int i=0; i < count; i++) {
           //     if (s->halted == 2) { ... }
           //     op = FETCH_OP(s);
           //     ...
       i.e. there was NO interrupt poll here at all.
       ------------------------------------------------------------------ */
    if (s->irq_pending || s->firq_pending || (s->nmi_pending && s->nmi_armed))
        check_interrupts(s);
