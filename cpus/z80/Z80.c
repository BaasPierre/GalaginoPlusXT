...
void StepZ80(Z80 *R)
{
  ...
  I=OpZ80_INL(R->PC.W++);
  /* BUG FIX (2026-09-19, flstory "BAD SOUND PCB" investigation - see
     source/machines/flstory/task_plain_6.txt): ExecZ80()/RunZ80() (the
     batch execution APIs) charge the base opcode cost via Cycles[I] here,
     BEFORE dispatching into Codes.h (see this same pattern a few hundred
     lines up in ExecZ80()) - Codes.h's own per-case ICount adjustments
     (e.g. JR_NZ's "ICount-=5" on the taken branch) are only the EXTRA
     cost on top of this base, for instructions whose cost varies by
     outcome. StepZ80() was missing this base charge entirely, so any
     caller reading R->ICount deltas around StepZ80() (there was no such
     caller anywhere in this codebase before this bug fix) would see
     wildly undercounted cycle costs - confirmed directly: a tight loop of
     cheap instructions (LD HL,nn / DEC HL / LD A,H / OR L / JR NZ) was
     measured getting through ~3x more REAL cycles' worth of real-hardware
     time than it should have, in the flstory port's own cycle-based main-
     CPU pacing (added in the same fix), before this line was added. */
  R->ICount-=Cycles[I];
