...
public:
    machineBase() { }
    // Frees every buffer registered with ownAlloc(). Galagino Plus now
    // constructs only the ACTIVE machine (see machines.h / main.cpp:
    // machineSwitch) and deletes it on the way back to the menu, so a
    // machine's lazily heap-allocated ROM/gfx/scratch buffers must be
    // released here or they leak on every game switch. Machines register
    // each alloc via `p = (T*)ownAlloc(heap_caps_malloc(...))`.
    virtual ~machineBase() {
      for (unsigned char i = 0; i < owned_count; i++)
    ...
    virtual void start() { }
    // Called by emulation_stop() when the user leaves this machine for the
    // menu. Default does nothing; a machine that lazily heap-allocates large
    // scratch buffers can free them here so the heap is available to the
    // next machine (needed when several memory-heavy machines are enabled).
    virtual void stop() { }
    virtual void reset() {
    ...
    // Virtual so audio.cpp does not need to include gng.h.
    ...
    // machine is unaffected. Virtual for the same reason as the hooks above:
    // audio.cpp must not include the machine headers.
...
protected:
    ...
    // Register a heap buffer so ~machineBase() frees it. Returns the pointer
    // unchanged, so:  buf = (uint8_t*)ownAlloc(heap_caps_malloc(n, CAPS));
    // (heap_caps_malloc memory is freeable with plain free() on this target).
    void *ownAlloc(void *p) {
      if (p && owned_count < (unsigned char)(sizeof(owned) / sizeof(owned[0])))
        owned[owned_count++] = p;
      return p;
    }
    // Drop a buffer from the owned list (call right after you free() it
    // yourself, e.g. a machine that re-allocates scratch in stop()/reset()).
    void ownForget(void *p) {
      for (unsigned char i = 0; i < owned_count; i++)
        if (owned[i] == p) { owned[i] = owned[--owned_count]; return; }
    }
...
#endif
    // owned heap buffers, freed by ~machineBase() (see ownAlloc)
    void *owned[8] = { 0 };
    unsigned char owned_count = 0;
};
