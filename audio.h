...
#ifdef DEBUG_TIMING
  // Diagnostic-only (2026-09-20, flstory audio-pileup investigation):
  // counts how many times transmit()'s own do/while(bytesOut) loop actually
  // iterates, and how many microseconds i2s_write() itself accounts for
  // across those iterations - isolates "looping many times" (DMA catch-up
  // after a long stall) from "one call is itself slow" (driver/lock
  // blocking despite ticks_to_wait=0). Machine-agnostic, main.cpp reads
  // and resets these every 1s alongside its existing render/write/audio
  // sums.
  unsigned long dbg_transmit_iters = 0;
  unsigned long dbg_i2s_write_us = 0;
#endif
...
private:
  ...
  signed char machineType = MCH_MENU;

#if defined(PCM5102_DAC) || defined(SND_DIFF)
  ...
  // Decoded shape control, mirroring MAME's ay8910.h::set_shape() exactly
  // (the previous code inferred these inline from the shape number and got
  // HOLD/ALTERNATE swapped - see the BUG FIX comment in audio.cpp). The
  // envelope counts step DOWN from 15 and XORs with attack, so a rising
  // shape is just a falling one with attack=15.
  int ay_envelope_attack[NUM_AY_CHIPS];
  int ay_envelope_hold[NUM_AY_CHIPS];
  int ay_envelope_alternate[NUM_AY_CHIPS];
