...
void Audio::start(machineBase *machineBase) {
  ...
  if (!machineBase) {          // detach (returning to the menu)
    machineType = MCH_MENU;
    AY = 0;
    return;
  }
  ...
  else if (machineType == MCH_CM99)       { AY = 1; AY_INC = 8; AY_VOL = 7;  }
  else if (machineType == MCH_PEPP0127)   { AY = 1; AY_INC = 8; AY_VOL = 7;  }
...
void Audio::transmit() {
  // No active machine (sitting in the menu): nothing to render. The render
  // branches below dereference currentMachine.
  if (!currentMachine)
    return;
...
void Audio::ay_render_buffer(void) {
  ...
  for(int i = 0; i < 64; i++) {
    ...
    for(char ay = 0; ay < AY; ay++) {
      ...
      // Elabora il generatore di rumore (R6).  R6 == 0 is the fastest noise
      // rate on real AY (treated as 1), not "noise off" - clamp so a
      // noise-only channel with R6 left at 0 still produces hiss.
      {
        int noise_period = ay_period[ay][3] ? ay_period[ay][3] : 1;
        audio_cnt[ay][3] += AY_INC;
        if(audio_cnt[ay][3] > noise_period) {
          audio_cnt[ay][3] -= noise_period;
          // progress rng
      ...
      for(char c = 0; c < 3; c++) {
        // A channel is audible if it has volume (fixed or envelope) AND R7
        // routes tone and/or noise to it (ay_enable != 0).  Do NOT also require
        // a non-zero tone period: a noise-only channel (tone off, noise on,
        // tone period left at 0) must still be heard - that is the "grrr" in
        // the pbaction startup jingle, and on real AY hardware a 0 tone period
        // is simply the highest tone frequency, not silence.
        // if((ay_period[ay][c] || ay_envelope[ay][c]) && ay_volume[ay][c] && ay_enable[ay][c]) {
        if(ay_volume[ay][c] && ay_enable[ay][c]) {
    ...
    // Ghosts'n Goblins: mix the 2x YM2203 FM (music) over the SSG effects.
...
// Internet radio: renderFmSample() is the ENTIRE audio contract for this
// machine (no chip emulation - it just pops one already-scaled-to-+/-512
// PCM sample per call from radio's own ring buffer, see radio.cpp/.h). This
// dedicated dispatch branch was previously missing from transmit()'s
// if/else chain entirely (MCH_RADIO matched none of AY/hasNamcoAudio/the
// named machineType checks), so renderFmSample() was never called and no
// radio audio ever reached the DAC regardless of decoder correctness -
